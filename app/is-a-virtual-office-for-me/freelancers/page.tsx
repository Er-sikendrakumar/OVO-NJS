...
export default async function Page() {
  ...
  const jsonPath = path.join(process.cwd(), "newsite/json/industry/freelancers.json");
  const jsonContent = await readFile(jsonPath, "utf-8");
  const freelancersData = JSON.parse(jsonContent);

  const FEATURES: Feature[] = freelancersData.features.map((feature: any) => ({
    title: normalizeCopy(feature.title),
    description: normalizeCopy(feature.description),
  }));

  const INDUSTRY_SECTIONS = freelancersData.industry_content.map((section: any) => ({
    main_heading: normalizeCopy(section.main_heading),
    industry_paragraph: normalizeCopy(section.industry_paragraph),
  }));

  const heroHeading = normalizeCopy(freelancersData.hero.heading);
  const heroDescriptionHtml = normalizeCopy(freelancersData.hero.description);
  const heroImage = freelancersData.hero.image;

  const servicesHeading = normalizeCopy(freelancersData.services.heading);
  const servicesDescriptionHtml = normalizeCopy(
    freelancersData.services.description
  );
  const servicesImage = freelancersData.services.image;
