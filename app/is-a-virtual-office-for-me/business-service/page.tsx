...
export default async function Page() {
  ...
  const jsonPath = path.join(process.cwd(), "newsite/json/industry/business-service.json");
  const jsonContent = await readFile(jsonPath, "utf-8");
  const businessserviceData = JSON.parse(jsonContent);

  const FEATURES: Feature[] = businessserviceData.features.map((feature: any) => ({
    title: normalizeCopy(feature.title),
    description: normalizeCopy(feature.description),
  }));

  const INDUSTRY_SECTIONS = businessserviceData.industry_content.map((section: any) => ({
    main_heading: normalizeCopy(section.main_heading),
    industry_paragraph: normalizeCopy(section.industry_paragraph),
  }));

  const heroHeading = normalizeCopy(businessserviceData.hero.heading);
  const heroDescriptionHtml = normalizeCopy(businessserviceData.hero.description);
  const heroImage = businessserviceData.hero.image;

  const servicesHeading = normalizeCopy(businessserviceData.services.heading);
  const servicesDescriptionHtml = normalizeCopy(
    businessserviceData.services.description
  );
  const servicesImage = businessserviceData.services.image;
