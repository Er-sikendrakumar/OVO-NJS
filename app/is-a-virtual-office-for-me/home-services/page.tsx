...
export default async function Page() {
  ...
  const jsonPath = path.join(process.cwd(), "newsite/json/industry/home-services.json");
  const jsonContent = await readFile(jsonPath, "utf-8");
  const homeServicesData = JSON.parse(jsonContent);

  const FEATURES: Feature[] = homeServicesData.features.map((feature: any) => ({
    title: normalizeCopy(feature.title),
    description: normalizeCopy(feature.description),
  }));

  const INDUSTRY_SECTIONS = homeServicesData.industry_content.map((section: any) => ({
    main_heading: normalizeCopy(section.main_heading),
    industry_paragraph: normalizeCopy(section.industry_paragraph),
  }));

  const heroHeading = normalizeCopy(homeServicesData.hero.heading);
  const heroDescriptionHtml = normalizeCopy(homeServicesData.hero.description);
  const heroImage = homeServicesData.hero.image;

  const servicesHeading = normalizeCopy(homeServicesData.services.heading);
  const servicesDescriptionHtml = normalizeCopy(
    homeServicesData.services.description
  );
  const servicesImage = homeServicesData.services.image;
