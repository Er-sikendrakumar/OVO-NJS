...
export default async function Page() {
  ...
  const jsonPath = path.join(process.cwd(), "newsite/json/industry/small-business.json");
  const jsonContent = await readFile(jsonPath, "utf-8");
  const smallbusinessData = JSON.parse(jsonContent);

  const FEATURES: Feature[] = smallbusinessData.features.map((feature: any) => ({
    title: normalizeCopy(feature.title),
    description: normalizeCopy(feature.description),
  }));

  const INDUSTRY_SECTIONS = smallbusinessData.industry_content.map((section: any) => ({
    main_heading: normalizeCopy(section.main_heading),
    industry_paragraph: normalizeCopy(section.industry_paragraph),
  }));

  const heroHeading = normalizeCopy(smallbusinessData.hero.heading);
  const heroDescriptionHtml = normalizeCopy(smallbusinessData.hero.description);
  const heroImage = smallbusinessData.hero.image;

  const servicesHeading = normalizeCopy(smallbusinessData.services.heading);
  const servicesDescriptionHtml = normalizeCopy(
    smallbusinessData.services.description
  );
  const servicesImage = smallbusinessData.services.image;
