...
const rawbase = "https://www.opusvirtualoffices.com"
const DEFAULT_IMAGE_ORIGIN = "https://www.opusvirtualoffices.com/newsite";
...
const normalizeUrlforslider = (url?: string) => {
  if (!url) return "";
  if (url.startsWith("http://") || url.startsWith("https://")) return url;
  if (url.startsWith("//")) return `https:${url}`;
  if (url.startsWith("/")) return `${rawbase}${url}`;
  return `${rawbase}/${url}`;
};
...
export function SpecialLocationSlider({
  ...
  const badgeImage = badgeImageSrc
    ? normalizeUrl(badgeImageSrc)
    : "/bestprice.svg";
  const normalizedLocations: Location[] = locations.map((location, index) => {
    ...
    return {
      ...
      image: normalizeUrlforslider(location.image),
    };
  });
  ...
  return (
    <section className="mx-auto w-full max-w-[1280px] px-4 py-5 md:px-8 md:pt-10  md:pb-[50px]">
          ...
          <div className="relative h-[191px] w-[283px] max-w-full md:h-[352px] md:w-full md:max-w-[608px]">
            {current.image ? (
              <img
                ...
                className="h-full w-full object-cover rounded-t-0 md:rounded-l-[12px] md:rounded-t-0"
              />
            ) : null}
          ...
          <div className="flex w-full max-w-[608px] flex-col justify-between gap-4 px-4 pt-3 pb-5 pr-4 md:ps-10">
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="flex items-center justify-between gap-4">
                <Link href={`/signup/?btn=6&locid=${current.id}`}>
                <button
                  className="inline-flex h-[36px] w-full md:w-auto  items-center justify-center rounded-lg bg-[#36BFFA] px-3 py-2 text-sm font-semibold text-white transition hover:bg-[#159fdc] md:h-[60px] md:min-w-[228px] md:px-5 md:py-4 md:text-[18px] md:leading-7"
                >
                ...
                <div className="hidden gap-4 md:flex">
                  <button
                    ...
                    className="flex h-12 w-12 items-center justify-center rounded-full border border-[#EAECF0] bg-white rotate-180"
                  >
                    <svg
