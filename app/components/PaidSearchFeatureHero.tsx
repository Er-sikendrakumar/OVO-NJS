...
import { GoogleRating } from "../paid-search/components/GoogleRating";
...
export function SpecialSearchFeature({
  ...
  const filteredLocations = useMemo(() => {
    if (!normalizedQuery || locations.length === 0) {
      return [];
    }
    ...
    return locations
      ...
      .slice(0, maxResults);
  }, [locations, maxResults, normalizedQuery]);

  const displayHeadline =
    typeof headline === "string"
      ? headline.replace("{search-term}", searchValue || "your city")
      : headline;
  ...
  return (
    <section className="flex w-full flex-col  self-stretch pb-5 md:pb-10 pt-5 md:pt-10 max-w-[1280px] px-4 lg:px-8 gap-2">
      ...
      <div className="flex w-full flex-col items-start gap-2 max-w-full md:max-w-[580.5px]">
        ...
        {normalizedQuery && filteredLocations.length > 0 ? (
          <div className="w-full rounded-[8px] border border-[#EAECF0] bg-white shadow-[0_10px_30px_-20px_rgba(16,24,40,0.35)]">
            <ul className="max-h-[240px] overflow-y-auto py-2">
              {filteredLocations.map((location) => {
                ...
                return (
                  <li key={location.id ?? label}>
                    {location.link ? (
                      <a
                        href={location.link}
                        className="block px-4 py-2 text-sm text-[#101828] hover:bg-[#F2F4F7]"
                      ...
                      <button
                        type="button"
                        className="block w-full px-4 py-2 text-left text-sm text-[#101828] hover:bg-[#F2F4F7]"
                        onMouseDown={(event) => event.preventDefault()}
                        onClick={() => setSearchValue(label)}
                      >
                        {label}
                      </button>
                    )}
