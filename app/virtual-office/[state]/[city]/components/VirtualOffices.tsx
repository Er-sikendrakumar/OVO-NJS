...
function VirtualOffices({ locations, city }: VirtualOfficesProps) {
    const mostPopularLocations = locations.filter(loc => loc.opusowned === 1);

    const otherLocations = locations.filter(loc => loc.opusowned !== 1);

    const [selectedLocation, setSelectedLocation] = useState<LocationData | null>(mostPopularLocations[0] || null);
    const [showAllFeatures, setShowAllFeatures] = useState(false);
    const showInfoWindowRef = useRef<((locationId: number) => void) | null>(null);

    return (
        <div className='max-w-screen-xl mx-auto px-[16px] lg:px-[32px] mt-[24px] lg:mt-[27px] lg:pb-[40px] pb-[0px] flex flex-col lg:flex-row lg:gap-[32px] gap-[24px]'>
            <div className='w-full flex flex-col gap-[24px]'>
                {mostPopularLocations.length > 0 && (
                    <>
                        <h2 className='font-inter font-semibold text-[20px] leading-[120%] text-[#101828]'>Most Popular Virtual Office Location in {city}</h2>
                                ...
                                >
                                    <div className='w-full lg:w-[239px] min-w-[239px] lg:min-h-[184px] min-h-[200px] h-full relative overflow-hidden'>
                                        <Image
                                            ...
                                            alt={location.image.alttext || `Virtual Office in ${location.city}`}
                                            className='object-cover object-center'
                                        />

                                        <div className='items-center flex flex-row gap-[8px] rounded-[12px] absolute top-[11px] left-[11px] z-10 w-[140px] h-[36px]' style={{ padding: '4px 8px 4px 4px', background: 'linear-gradient(45deg, #FF7A00 0%, #FFD439 100%)' }}>
                                            <div className='px-[10px] py-[2px] bg-white rounded-[9px] flex text-center justify-center items-center min-w-[58px] min-h-[28px]'>
                                                <span className='text-[#252B37] font-medium text-[16px] leading-[100%] tracking-[0]'>Most</span>
                                            </div>
                                            <span className='text-white font-bold text-[16px] leading-[100%] tracking-[0]'>Popular</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className='p-[24px] flex flex-col gap-[20px] justify-between w-full'>
                                        <div className='flex flex-col gap-[4px] w-full'>
                                            <div className='flex flex-row w-full justify-between items-center'>
                                                <span className='font-bold text-[24px] leading-[120%] tracking-[0] text-black'>{location.city}</span>
                                                {location.premium === 1 && (
                                                    <div className='pl-[4px] pr-[8px] py-[4px] rounded-full border border-[#75E0A7] bg-[#ECFDF3] flex flex-row gap-[8px] items-center'>
                                                        <div className='border border-[#75E0A7] bg-white rounded-full px-[8px] py-[2px] flex justify-center items-center text-center'>
                                                            <span className='text-[#067647] text-[12px] leading-[100%] tracking-[0] font-medium'>Premium</span>
                                                        </div>
                                                        <span className='text-[#067647] text-[12px] leading-[100%] tracking-[0] font-medium'>Location</span>
                                                    </div>
                                                )}
                                            </div>
                                        ...
                                        <div className='w-full bg-[#36BFFA] rounded-[8px] px-[14px] py-[10px] flex justify-center items-center text-center
                ...
                {otherLocations.length > 0 && (
                    <>
                        <h2 className='font-inter font-semibold text-[20px] leading-[120%] text-[#101828]'>Additional Virtual Office Locations in {city}</h2>
            ...
            <div className='w-full flex flex-col gap-[36px] lg:sticky lg:top-[100px] lg:self-start'>
                ...
                <div>
                    <div className="text-base font-normal mb-3 max-w-[213px] text-[#475467]">
                        When We Say <span className='font-semibold'>All-Inclusive</span>,  We Mean <span className='font-semibold'>ALL-INCLUSIVE</span>:
                    </div>
                    ...
                    <div className="space-y-3">
                        {features.map((feature, index) => (
                            ...
                            >
                                ...
                                <span className="text-[16px] lg:text-lg font-normal leading-[100%] text-[#475467]">{feature}</span>
                            </div>
                        ))}
                    </div>

                    {/* Show "See All Features" button on mobile only when not expanded */}
