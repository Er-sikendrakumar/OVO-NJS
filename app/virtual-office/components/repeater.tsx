        ...
        <>
            {items.map((item, index) => {
                ...
                if (item.layout === "text_only" && item.body) {
                    ...
                    const bodyText = replacePlaceholders(item.body, state, cityName);
                    ...
                    return (
                        <LocationsContent
                            key={`text-only-${index}`}
                            heading={replacePlaceholders(item.title || "", state, cityName)}
                ...
                if (item.layout === "image_text" && item.title && item.body && item.image) {
                    ...
                    // Extract image URL from string or object
                    const getImagePath = (img: string | { url: string; alt?: string; title?: string }): string => {
                    ...
                    return (
                        <TextWithImage
                            ...
                            title={replacePlaceholders(item.title, state, cityName)}
                            body={replacePlaceholders(item.body, state, cityName)}
