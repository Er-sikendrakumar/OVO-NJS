...
async function fetchCategoryBySlug(
  ...
  try {
    ...
    const apiUrl = `${baseUrl}/wp-json/wp/v2/categories?slug=${encodeURIComponent(
      slug
    )}`;
...
async function fetchPostsByCategory(
  ...
  try {
    ...
    const apiUrl = `${baseUrl}/wp-json/wp/v2/posts?categories=${categoryId}&per_page=${perPage}&page=${page}&_embed=true&orderby=date&order=desc`;
...
export async function generateMetadata({
  ...
  return {
    ...
    description: category.description || `Recent posts about ${category.name}`,
  };
}
...
export default async function CategoryPage({
  ...
  const blogData = await fetchPostsByCategory(
    category.id,
    currentPage,
    postsPerPage
  );
    ...
    <div className="bg-white flex flex-col">
      ...
      <div className="pt-[92px] md:pt-[144px] pb-[20px] md:pb-[40px] w-full flex-1 max-w-[1280px] mx-auto px-[16px] md:px-[32px]">
        ...
        <p className="text-[20px] text-[#535862] font-normal ">
          Recent Posts About {category.name}
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return pages.map((page, index) => {
                    ...
                    return (
                      <Link
                        key={page}
                        href={`/blog/category/${id}?page=${page}`}
